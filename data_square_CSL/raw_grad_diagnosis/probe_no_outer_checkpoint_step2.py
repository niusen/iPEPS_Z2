...
import ctmrg.bosonic_CTMRG_unitcell_iPEPS as ctm_module
...
ctm_module.checkpoint = direct_checkpoint
...
def eval_loss(parameters, state, ctm_args, global_args, config_kwargs):
    CTM_cell, double_A_cell, ite_num, ite_err = ctm_module.Bosonic_CTMRG_cell_iPEPS(
...
def main():
    ...
    print("outer_checkpoint,disabled", flush=True)
    ...
    print("after_step1_raw_param_norm," + "{:.16g}".format(param.detach().norm().item()), flush=True)
