...
def compute_grad(label, template_tensor, param, parameters, ctm_args, global_args, config_kwargs, optimizer=None):
...
def main():
    ...
    optimizer = build_optimizer(param, args)
    ...
    def closure_linesearch(linesearching=True):
        ...
        print("line_search_E," + "{:.16g}".format(to_float(loss)), flush=True)
        return loss

    optimizer.step_2c(closure, closure_linesearch)
    raw_after_step1 = param.detach().clone()
    print("after_step1_raw_param_norm," + "{:.16g}".format(raw_after_step1.norm().item()), flush=True)

    compute_grad(
        "step2_same_param_optimizer_zero",
        template_tensor,
        param,
        parameters,
        ctm_args,
    ...
    cloned_param = torch.nn.Parameter(raw_after_step1.clone())
    compute_grad(
        "step2_cloned_param_no_optimizer",
        template_tensor,
        cloned_param,
        parameters,
        ctm_args,
    ...
    cloned_param_2 = torch.nn.Parameter(raw_after_step1.clone())
    cloned_optimizer = build_optimizer(cloned_param_2, args)
    compute_grad(
        "step2_cloned_param_new_optimizer",
        template_tensor,
        cloned_param_2,
        parameters,
        ctm_args,
    ...
    reloaded_state = IPEPS_SQUARE_C4_PT(load_square_iPEPS(args.instate_prefix, config_kwargs), config_kwargs)
    reloaded_state.normalize()
    reloaded_template = reloaded_state.A_set["1,1"].copy()
    cloned_param_3 = torch.nn.Parameter(raw_after_step1.clone())
    compute_grad(
        "step2_reloaded_template_cloned_param",
        reloaded_template,
        cloned_param_3,
        parameters,
        ctm_args,
