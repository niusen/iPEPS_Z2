...
U=0;
mx=0;
parameters={"t1": t1, "t2": t2, "蠒": 蠒, "渭":  渭, "U":  U, "mx":  mx};
print(parameters)
energy_setting=Square_Hubbard_Energy_settings();
energy_setting.model = 'triangle_spinlessHofstadter';
...
with torch.no_grad():
    ...
    for chi in chis:
        ...
        mat_filenm="ob_spinless_D"+str(D)+"_chi"+str(chi);
        datadic = {"E_total": E_total.item(), "e0_set": e0_set.cpu().numpy(), "ex_set": ex_set.cpu().numpy(), "ey_set": ey_set.cpu().numpy(), "e_diagonala_set":e_diagonala_set.cpu().numpy() }
        savemat(mat_filenm+".mat", datadic)

        distance=40;
        partly=True;
        CdagC_ob_set=cal_correl_spinless(CTM_cell,B_set,T_set,double_B_set, double_T_set,D,chi,'x',distance, config_kwargs, global_args, partly);
