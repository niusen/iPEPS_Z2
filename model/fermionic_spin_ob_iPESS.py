"""J1-Jchi energy for a d=2 Z2-symmetric fermionic triangular iPESS."""

import torch
import yastn
from torch.utils.checkpoint import checkpoint

from ansatz.fermionic_spin_triangle_iPESS import fermionic_z2_config
...
def _matrix_to_odd_operator(matrix, config_kwargs):
    config = fermionic_z2_config(config_kwargs)
    leg_out = yastn.Leg(config, s=1, t=(1,), D=(2,))
    leg_in = yastn.Leg(config, s=-1, t=(1,), D=(2,))
    operator = yastn.zeros(config=config, legs=[leg_out, leg_in])
    operator.set_block(ts=(1, 1), Ds=(2, 2), val=matrix)
    return operator


def spin_half_components_fermionic_d2(config_kwargs):
    """Return physical Sx, Sy, Sz on ``(|up>, |down>)``."""
    ...
    return tuple(
        _matrix_to_odd_operator(matrix, config_kwargs)
        for matrix in (sx, sy, sz)
    )


def spin_half_operators_fermionic_d2(config_kwargs):
    """Factorize S.S and scalar chirality in the exact d=2 space."""
    Sx, Sy, Sz = spin_half_components_fermionic_d2(config_kwargs)
    SS = (
    ...
    return (
        Sa, Sb, SS_string,
        chirality_S1, chirality_S2, chirality_S3,
        string12, string23,
    )


def evaluate_fermionic_triangle_spin_energy(
    parameters,
    B_set,
    T_set,
    double_B_set,
    ...
    """Evaluate energy/site for the d=2 fermionic-iPESS spin state.

    On-site magnetization is intentionally evaluated only when
    ``return_observables=True`` (the accepted line-search path), never during
    gradient evaluation.
    """
    J1 = parameters.get("J1", 1.0)
    Jchi = parameters.get("Jchi", parameters.get("J_chi", 0.0))
    Sa, Sb, SS_string, S1, S2, S3, string12, string23 = (
        spin_half_operators_fermionic_d2(config_kwargs)
    )
    Lx, Ly = global_args.Lx, global_args.Ly
    values = {
        "chi_up": [], "chi_down": [], "SS_x": [],
        "SS_y": [], "SS_diagonal": [],
    }
    if return_observables:
        values.update({"Sx": [], "Sy": [], "Sz": []})
        Sx_operator, Sy_operator, Sz_operator = (
            spin_half_components_fermionic_d2(config_kwargs)
        )
    energy = 0.0
    ...
    for cx in range(1, Lx + 1):
        for cy in range(1, Ly + 1):
            ...
            # ob_dn uses the opposite cyclic ordering; this minus sign gives
            # up and down triangles the same Hamiltonian chirality convention.
            chi_down = -checkpoint(
                ob_dn_triangle_iPESS,
                CTM_cell, S1, S2, S3, string12, string23,
                B_set, T_set, double_B_set, double_T_set,
            ...
            if return_observables:
                # This branch is called only after line search accepts a step.
                with torch.no_grad():
