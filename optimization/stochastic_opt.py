...
def stochastic_opt(x0, delta, maxiter, gtol):
    global chi,D
    print("stochastic optimization")
    print("D="*string(D));flush(stdout);
    print("chi="*string(chi));flush(stdout);
...
def fx(x):
    ...
    if isa(x[1],Triangle_iPESS)
        if energy_setting.model == "spinful_triangle_lattice";
            ...
            print("E= "*string(E)*", "*"ex_set= "*string(ex_set[:])*", "*"ey_set= "*string(ey_set[:])*", "*"e_diagonal1_set= "*string(e_diagonal1_set[:])*", "*"e0_set= "*string(e0_set[:])*", "*"eU_set= "*string(eU_set[:])*", "*"ctm_ite_num= "*string(ite_num)*", "*"ctm_ite_err= "*string(ite_err));flush(stdout);
            print("occu="*string(sum(e0_set)/length(e0_set)));flush(stdout);
        elseif energy_setting.model == "standard_triangle_Hubbard";
            ...
            print("ctm_ite_num= "*string(ite_num)*", "*"ctm_ite_err= "*string(ite_err));flush(stdout);
            print("E= "*string(E));flush(stdout);
            print("ex_set= "*string(ex_set[:])); flush(stdout);
            print("ey_set= "*string(ey_set[:]));flush(stdout);
            print("e_diagonal1_set= "*string(e_diagonal1_set[:]));flush(stdout);
            print("e0_set= "*string(e0_set[:]));flush(stdout);
            print("occu="*string(sum(e0_set)/length(e0_set)));flush(stdout);
            print("eU_set= "*string(eU_set[:])); flush(stdout);
            print("triangle_up_set= "*string(triangle_up_set[:])); flush(stdout);
            print("triangle_dn_set= "*string(triangle_dn_set[:])); flush(stdout);
    ...
    if E<minimum(E_history)
        ...
        print("Time consumed: "*string(Time));flush(stdout);
    end
    return E;
end
