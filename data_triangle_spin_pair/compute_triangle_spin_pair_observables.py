"""Compute observables of a saved d=4 triangular chiral-pair iPESS.
...
state_file = "triangle_spin_pair_J1_1_JchiA_0p4_JchiB_m0p4_D4_chi40_Lx2_Ly2"
...
def print_observables(energy, observables):
    print("E (average per layer)= " + str(energy.item()))
...
def main():
    ...
    if state.T_set["1,1"].get_shape()[1] != 4:
    ...
    with torch.no_grad():
        ...
        energy, observables = pair_model.evaluate(
            parameters,
            state.B_set,
            state.T_set,
            double_B_set,
