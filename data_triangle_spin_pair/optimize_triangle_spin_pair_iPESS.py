"""Optimize two decoupled triangular J1-Jchi layers with opposite Jchi.

Edit the parameters in this file, then submit it with
``run_triangle_spin_pair_server.sh``.  This intentionally follows the original
...
parameters = {
    "J1": J1,
    "Jchi_chiral": Jchi_chiral,
    "Jchi_antichiral": Jchi_antichiral,
}
...
initial_state_file = "initial_triangle_spin_pair_D4"
...
save_file_name = (
    ...
    + "_JchiA_" + parameter_tag(Jchi_chiral)
    + "_JchiB_" + parameter_tag(Jchi_antichiral)
    + "_D" + str(D)
    + "_chi" + str(chi)
    + "_Lx" + str(Lx)
    + "_Ly" + str(Ly)
...
def main():
    ...
    # Construct U and all U-dependent operators once. Every gradient,
    # line-search and observable call reuses this exact model object.
    split_unitary = fixed_d4_to_two_spin_unitary(
    ...
    if load_initial_state:
        ...
        if state.T_set["1,1"].get_shape()[1] != physical_dim:
            raise ValueError("loaded chiral-pair state must have physical dimension d=4")
    ...
    energy_setting.model = pair_model.model_name
    energy_setting.model_object = pair_model
